...
class dataUploadView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            form.save()
            data_ca= request.POST.get('Customer_Age')
            data_gd=request.POST.get('Gender')
            data_el=request.POST.get('Education_Level')
            data_ict=request.POST.get('Income_Category')
            data_cc=request.POST.get('Card_Category')
            data_cl=request.POST.get('Credit_Limit')
            data_tta=request.POST.get('Total_Trans_Amt')
            #print (data)
            ...
            data = np.array([data_ca,data_gd,data_el,data_ict,data_cc,data_cl,data_tta])
            #sc = StandardScaler()
...
#data_for_prediction =obj.pca(np.array(data_for_prediction),y_test)
            ...
            return render(request, "succ_msg.html", {'data_ca':data_ca,'data_gd':data_gd,'data_el':data_el,'data_ict':data_ict,'data_cc':data_cc,'data_cl':data_cl,'data_tta':data_tta,
                                                        'out':out})
